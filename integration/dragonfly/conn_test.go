...
func TestClientPacketCancelledByKillLimeDoesNotReachDragonfly(t *testing.T) {
	raw := newBlockingConn()
...
func TestServerPacketIsProcessedBeforeSingleClientWrite(t *testing.T) {
	...
	case written := <-raw.writes:
		...
		if pk.Tick != 0 {
			t.Fatalf("MobEffect.Tick = %d, want KillLime-normalized 0", pk.Tick)
		}
	default:
...
func TestStartGameConnectsKillLimeOutputToDragonfly(t *testing.T) {
	raw := newBlockingConn()
	p := player.New(slog.Default(), player.MonitoringState{CurrentTime: time.Now()}, nil)
	component.Register(p)
	conn := newSessionConn(raw, p)
