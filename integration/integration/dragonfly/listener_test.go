...
func TestListenerFactoryListensOnEphemeralAddress(t *testing.T) {
	...
	l, err := factory(server.Config{
		...
		Name:           "KillLime test",
		StatusProvider: minecraft.NewStatusProvider("KillLime test", "KillLime test"),
	})
	if err != nil {
...
func TestWrappedListenerDisconnectDelegatesAndClosesKillLimePlayer(t *testing.T) {
	base := newStubListener()
	...
	case <-time.After(time.Second):
		t.Fatal("Disconnect did not close the KillLime player")
	}
}
...
func TestDisconnectClosesKillLimePlayerLifecycle(t *testing.T) {
	p := player.New(slog.Default(), player.MonitoringState{CurrentTime: time.Now()}, nil)
