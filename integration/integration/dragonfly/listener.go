// Package dragonfly integrates KillLime directly into a Dragonfly server without
...
// Config configures the native KillLime listener used by Dragonfly.
type Config struct {
	...
	// Configure is called after KillLime's default components and detections have
	// been registered for a player.
	Configure func(*player.Player)
}

// Listener returns a Dragonfly listener factory backed by an KillLime player
...
func Listener(ctx context.Context, cfg Config) func(server.Config) (server.Listener, error) {
	return func(conf server.Config) (server.Listener, error) {
		...
		log.Info("Dragonfly with KillLime listening", "addr", raw.Addr())
		return newListener(ctx, nil, raw, log, cfg.Configure), nil
	}
}

// Wrap adds KillLime packet processing to a Dragonfly listener factory backed by
...
func (l *listener) Accept() (session.Conn, error) {
	...
	p.SetRecoverFunc(func(p *player.Player, v any) {
		l.log.Error("recovered panic in KillLime player pipeline", "player", p.IdentityDat.DisplayName, "panic", v)
		_ = p.Close()
	})
	p.SetConn(conn)
