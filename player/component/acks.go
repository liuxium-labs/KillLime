...
// Pending returns the number of acknowledgments that have not yet been acknowledged by the
// member player. This is useful for determining whether the client is tampering with the
// NetworkLatency acknowledgment packets, as a legitimate response to one of KillLime's
// acknowledgments will always match a pending batch.
func (ackC *ACKComponent) Pending() int {
	return len(ackC.pending)
}
...
// player. This is likely to be used in KillLime recodings/replays.
