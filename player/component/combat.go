...
func (c *AuthoritativeCombatComponent) Attack(input *packet.InventoryTransaction) {
	...
	// The client tracker mirrors the client's view of every entity (players,
	// mobs, end crystals, ...), so all of them are validated. Reach detections
	// need this to catch e.g. crystal aura attacks on non-player entities.
	if c.useClientTracker && e.IsPlayer {
		c.mPlayer.Dbg.Notify(player.DebugModeCombat, true, "client-tracker attack on player entity %d", data.TargetEntityRuntimeID)
	}
...
func (c *AuthoritativeCombatComponent) Calculate() bool {
	...
	// Creative (and spectator) mode trusts the client and always forwards the
	// hit, but the distance calculation below still runs so that hooks
	// (reach/hitbox detections) can evaluate the attack. This keeps the
	// detections working in creative mode instead of being skipped entirely.
	gamemodeBypass := c.mPlayer.GameMode != packet.GameTypeSurvival && c.mPlayer.GameMode != packet.GameTypeAdventure
	...
	if gamemodeBypass {
		// Creative/spectator hits are trusted: always forward, but the hooks
		// below still get the measured distances to evaluate the attack.
		forwardHit = true
		c.mPlayer.Dbg.Notify(
			player.DebugModeCombat,
			true,
			"<yellow>gamemode bypass: forwarding hit</yellow> (raycast=%f, raw=%f, angle=%f)",
			closestRaycastDist,
			closestRawDist,
			closestAngle,
		)
	...
	return gamemodeBypass || !c.checkMisprediction || hitValid
}
