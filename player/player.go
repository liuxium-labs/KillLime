...
import (
	...
	"github.com/killlime/killlime/world"
	"github.com/killlime/killlime/world/blocknetwork"
...
type Player struct {
	...
	// KillLime has sent to it. This is used to not spam the client with mutliple corrections which may
	...
	// make an attack packet when KillLime's full authoritative combat detects a misprediction.
...
// SendRemoteEvent sends an KillLime event to the remote server.
...
// NMessage sends a message to the player without the KillLime prefix.
...
func (p *Player) BlockAddress(duration time.Duration) {
	...
	// KillLime cannot access transport-specific listener state.
	if p.listener == nil {
		return
	}
