...
import (
	...
	"github.com/killlime/killlime/player/context"
	"github.com/killlime/killlime/utils"
	oworld "github.com/killlime/killlime/world"
...
func (p *Player) HandleClientPacket(ctx *context.HandlePacketContext) {
	...
	case *packet.ScriptMessage:
		...
		if strings.Contains(pk.Identifier, "KillLime:") {
			p.Disconnect("\t")
			return
		}
	case *packet.PlayerAuthInput:
		if !p.movement.InputAcceptable() {
			...
			// Rate-limited inputs are still scanned by detections (e.g. TimerA
			// relies on seeing every PlayerAuthInput to detect input flooding).
			p.RunDetections(pk)
			ctx.Cancel()
			return
		}

		// Since KillLime utilizes a full-authoritative system for movement, we are always modifying the position in PlayerAuthInput packet
		// to KillLime's predicted position.
		ctx.SetModified()
		...
		if tr, ok := pk.TransactionData.(*protocol.UseItemOnEntityTransactionData); ok {
			...
			if tr.ActionType == protocol.UseItemOnEntityActionAttack {
				if p.GameMode == packet.GameTypeSurvival || p.GameMode == packet.GameTypeAdventure {
					// The reason we cancel here is because KillLime also utlizes a full-authoritative system for combat. We need to wait for the
					...
					ctx.Cancel()
				} else if p.opts.Combat.EnableClientEntityTracking {
					// Creative/spectator attacks are forwarded untouched, but the client
					// combat component is still fed so reach/hitbox detections can
					// evaluate the attack instead of being completely blind in creative.
					p.ClientCombat().Attack(pk)
				}
			}
