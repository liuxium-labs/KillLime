...
import (
	...
	killlime_block "github.com/killlime/killlime/world/block"
...
func SimulatePlayerMovement(p *player.Player, movement player.MovementComponent) {
	...
	if supportPos := movement.SupportingBlockPos(); supportPos != nil {
		...
		if _, isAir := blockUnder.(block.Air); isAir {
			...
			if killlime_block.IsWall(b) || killlime_block.IsFence(b) {
				blockUnder = b
			}
		}
	}
...
func simulateGlide(p *player.Player, movement player.MovementComponent) {
	...
	// VERSION OF BOTH MCBE AND MCJE SHOWS), putting the logic here allowed KillLime's prediction to be accurate...
...
func tryCollisions(p *player.Player, src world.BlockSource, dbg *player.Debugger, useSlideOffset bool, clientJumpPrevented bool) {
	...
	if onGround && (xCollision || zCollision) {
		...
		if !hasStepCollisions && game.Vec3HzDistSqr(collisionVel) < game.Vec3HzDistSqr(stepVel) {
			...
			// is because in some scenarios, the client seems to reject a step even though KillLime thinks it is possible. This is mainly in scenarios
			...
			// collisionPosDist is within the correction threshold. Even if the stepPosDist is greater than the correction threshold, KillLime is predicting
