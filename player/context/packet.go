...
// HandlePacketContext is a context that is passed to KillLime's HandleClientPacket and HandleServerPacket functions.
type HandlePacketContext struct {
	// pk is a pointer to the underlying packet that is being handled.
	pk *packet.Packet
	// modified is a boolean indicating whether KillLime has modified the packet. If it has, we have to re-encode
