...
// acknowledgments to the player. Thpackage player
...
type AcknowledgmentComponent interface {
	...
	// Pending returns the number of acknowledgments that have not yet been acknowledged by
	// the member player.
	Pending() int
	...
	// player. This is likely to be used in KillLime recordings/replays.
...
is is vital for ensuring lag-compensated player experiences.
type AcknowledgmentComponent interface {
	// Add adds an acknowledgment to the current acknowledgment list of the ack component.
	Add(ack Acknowledgment)
	// Execute runs a list of acknowledgments with the given timestamp. It returns true if the ack ID
